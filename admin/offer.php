                  ...
                  <div class="row">
                      <div class="col-lg-12">
                          <h1 class="page-header">
                              Welcome to Admin <small>Here You can Manage All Offers</small>
                          </h1>
                          <?php
...
if (isset($_FILES['image'])) {
  ...
  if (!empty($_FILES['image']['tmp_name']) && getimagesize($_FILES['image']['tmp_name'])) {
    if ($_FILES['image']['size'] > 5000000) {
      ...
      $stmt = $pdo->prepare('INSERT INTO offer VALUES (NULL,?, CURRENT_TIMESTAMP)');
    ...
    img.onload = () => {
      ...
      image_popup.innerHTML = `
        <div class="con">
          ...
          <a href="offer.php?id=${img_meta.dataset.id}" >Delete</a>
        </div>
      `;
...
if (isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM offer WHERE id = ?');
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            unlink($image['image']);
            $stmt = $pdo->prepare('DELETE FROM offer WHERE id = ?');
            $stmt->execute([$_GET['id']]);
            // Output msg
